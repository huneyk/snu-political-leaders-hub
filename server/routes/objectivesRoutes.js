...
router.get('/', async (req, res) => {
  try {
    ...
    const objectives = await Content.find({ type: 'objectives' }).sort({ createdAt: -1 });
    
    // 데이터가 없는 경우 처리
    if (!objectives || objectives.length === 0) {
      console.log('목표 데이터가 없습니다.');
      return res.json([]); // 빈 배열 반환
    }
    
    console.log(`목표 정보 조회 성공: ${objectives.length}개 항목 발견`);
    res.json(objectives);
  } catch (error) {
...
router.post('/', isAdmin, async (req, res) => {
  try {
    ...
    const newObjective = new Content({
      type: 'objectives',
      title,
      content,
      ...req.body // 추가 필드
    ...
    res.status(500).json({ message: '목표 정보를 생성하는 중 오류가 발생했습니다.' });
  }
});
