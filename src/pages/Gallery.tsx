      ...
      if (Array.isArray(galleryData) && galleryData.length > 0) {
        ...
        const termInfos: TermGalleryInfo[] = validTerms.map(term => {
          ...
          if (items.length > 0) {
            ...
            // 갤러리 데이터가 없는 기수도 표시 (하지만 실제 존재하는 기수)
            return {
              term,
              count: 0,
              latestDate: new Date().toISOString(),
              thumbnailUrl: '' // CSS placeholder로 처리
            };
          }
        ...
        console.log('✅ 갤러리 메타데이터 로드 완료:', sortedTermInfos);
        ...
        const sortedTermInfos = termInfos.sort((a, b) => Number(b.term) - Number(a.term));
        setTermGalleries(sortedTermInfos);
        console.log('📋 갤러리 데이터는 없지만 유효한 기수들로 목록 생성:', sortedTermInfos);
      }
